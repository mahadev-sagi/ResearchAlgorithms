...
// --- VERIFICATION HARNESS (Paste at the very bottom of qs_02_dualpivot.cpp) ---
#include <iostream>
#include <vector>
#include <fstream>
...
int main() {
    ...
    if (!file.is_open()) {
        std::cout << "Warning: numbers.txt not found. Using small test set." << std::endl;
        arr = {5, 1, 9, 3, 7, 2, 8, 4, 6, 0};
    } else {
        while (file >> num) {
            arr.push_back(num);
        }
    ...
    // 2. Execution
    // Call the namespaced sort function
    dual_pivot_quicksort::sort(arr.begin(), arr.end());

    // 3. Verification
    bool passed = true;
    for (size_t i = 0; i < arr.size() - 1; ++i) {
        if (arr[i] > arr[i+1]) {
            std::cerr << "FAILED at index " << i << ": " << arr[i] << " > " << arr[i+1] << std::endl;
            passed = false;
            break; 
        }
    }
