...
__global__ void recursive_msd_kernel(unsigned int* data, int n, int bit) {
    if (n <= 1 || bit < 0) return;
    int tid = threadIdx.x;
    if (tid == 0) {
        ...
        if (buffer != NULL) {
            ...
            free(buffer);
            if (l > 1) recursive_msd_kernel<<<1, 1>>>(data, l, bit - 1);
            if ((n - l) > 1) recursive_msd_kernel<<<1, 1>>>(data + l, n - l, bit - 1);
        }
    }
}
...
int main() {
    ...
    recursive_msd_kernel<<<1, 1>>>(d_data, N, 31);
